...
### 4. Año bisiesto
Determinar si un año es bisiesto o no. Los años bisiestos son múltiplos de 4; utilícese el operador módulo. ¡Pero hay más excepciones! Los múltiplos de 100 no son bisiestos, aunque sean múltiplos de 4. Pero los múltiplos de 400 sí, aunque sean múltiplos de 100. Qué follón. La Tierra es muy maleducada al no ajustarse a los patrones de tiempo humanos. Resumiendo: un año es bisiesto si es divisible entre 4, a menos que sea divisible entre 100. Sin embargo, si un año es divisible entre 100 y además es divisible entre 400, también resulta bisiesto. Ahora, prográmalo, a ver qué sale.
...
### 7. Contar cifras
Determinar el número de cifras de un número entero. El algoritmo debe funcionar para números de hasta 5 cifras, considerando los negativos. Por ejemplo, si se introduce el número 5342, la respuesta del programa debe ser 4. Si se introduce –250, la respuesta debe ser 3. Para los quisquillosos: no, el 0 a la izquierda no cuenta.
...
### 18. Factorial
Calcular el factorial de un número entero N. Recuerda que el factorial de un número es el producto de ese número por todos los enteros menores que él. Por ejemplo, el factorial de 5 (simbolizado 5!) se calcula como: 5! = 5 x 4 x 3 x 2 x
...
### 23. Omirps
Un omirp es una de esas cosas que nos hace dudar sobre la estabilidad mental de los matemáticos. Se trata de un número primo que, al invertirlo, también da como resultado un número primo. Por ejemplo, el número 7951 es primo y, si le damos la vuelta, obtenemos 1597, que también es primo. Por lo tanto, 7951 es un omirp. Se trataría, pues, de introducir un número y que el programa determine si es un omirp o no. O más difícil todavía ( redoble de tambores): hacer un programa que muestre la lista de omirps entre 0 y N, siendo N un número introducido por el usuario.

### 24. Lotería primitiva
Generar combinaciones al azar para la lotería primitiva (6 números entre 1 y 49). Debes utilizar el método Math.random() que vimos en el tema 1. Por ahora, no te preocupes porque los números puedan repetirse. No hace falta que corras a la administración de loterías a jugar la primera combinación que te salga. Lo han probado muchas promociones de alumnos antes que tú y no nos consta que nadie haya conseguido salir de pobre.

### 25. Quiniela
Generar combinaciones al azar para la quiniela (14 valores dentro del conjunto 1, X o 2, por si hay alguien de otro planeta que no sepa cómo se rellena una quiniela). El resultado debe ser algo así, pero generado al azar: 1-X-X-2-1-1-1-2-2-X-1-X-X-2

### 26. Calculadora
Diseñar un algoritmo que lea dos números, A y B, y un operador (mediante una variable de tipo carácter), y calcule el resultado de operar A y B con esa operación. Por ejemplo, si A = 5 y B = 2, y operación = "+", el resultado debe ser 7. El algoritmo debe seguir pidiendo números y operaciones indefinidamente, hasta que el usuario decida terminar (utiliza un valor centinela para ello)
